...
void setup() {
  ...
  for (int i=0;i<2;i++) {
    ...
    if (WiFi.status() == WL_CONNECTED) {    //若連線成功
      ...
      Serial.println("");
      lcd.setCursor(0,0);   //wifi connected show IP address
      lcd.print("STAIP address:");
      lcd.setCursor(0,1);
      lcd.print(WiFi.localIP());
      Serial.println("Setting mDNS....");
      if(!MDNS.begin("esp32face")) {//這裡的名稱就是之後輸入瀏覽器會跳到你的網頁的名稱
         Serial.println("Error starting mDNS");
      }else{
        Serial.println("http://esp32face.local/");
      }
      #if defined(CAMERA_MODEL_AI_THINKER)
...
void loop() {
  if (cmd.length()>0) {       
      lcd.setCursor(0,0);    //螢幕Show影像辨識結果
    ...
    <body>
          ...
          <div class="collapse navbar-collapse" id="navbarSupportedContent">
            <ul class="navbar-nav ml-auto">
              <li class="nav-item active">
                <a class="nav-link">Back<span class="sr-only">(current)</span></a>
              </li>
          </div>
        </nav>
        <section class="main">
        ...
        <script>
          ...
          $(".nav-link").on('click',function(){
            $(location).attr('href', 'http://group3.local/');
            $(window).attr('location','http://group3.local/');
            $(location).prop('href', 'http://group3.local/');
          });
        </script>
        ...
        async function predict() {
          ...
          if (maxPredictions>0) {
            ...
            if(maxClassName=='Class 1' && maxProbability>=0.999){
              $(location).attr('href', 'http://group3.local/home');
              $(window).attr('location','http://group3.local/home');
              $(location).prop('href', 'http://group3.local/home');
            }
